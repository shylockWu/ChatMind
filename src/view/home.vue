...
import { apiClient} from './server/index';
...
currentIndex.value = this.messages.length - 1;
  this.$nextTick(() => {
  ...
  //  const AllRes = await Promise.all(AIRequest) // 可能耗时很久，可以考虑循环请求输出
    ...
    this.session_id = data.session_id; // 更新session_id
    this.newMessage = "";
  } catch (error) {
    console.log(error);
  }
...
<template>
  <div class="container">
    <h1>模拟与AutoGPT对话</h1>
    <div ref="list" class="chat-window">
      ...
      >
        <div v-if="!message.isBot" class="message-container reverse">
          <div class="user-avatar">
            <img
              loading="eager"
              src="https://avatars.githubusercontent.com/u/108790041?v=4"
              data-image-src="https://avatars.githubusercontent.com/u/108790041?v=4"
            >
          </div>
          <div class="user-message">
            {{ message.content
            }}<span v-show="index === currentIndex" class="typing-indicator"
            >_</span>
          </div>
        </div>
